...
export class Utils {
    ...
    public remove (path : string , { delayMs = 1000 * 60 * 60  } : { delayMs ?: number } = {}) {
    ...
    public async trashed ({ path , bucket } : { 
        path     : string
        bucket   : string
    }) {
        ...
        const currentPath = this.normalizePath({ directory : this.normalizeDirectory({ bucket , folder : null }) , path , full : true })
    ...
    public normalizeDirectory({ bucket , folder } : { bucket : string , folder ?: string | null }): string {
    ...
    public normalizePath({ directory , path , full = false } : { 
    ...
    public async makeStream ({ bucket , filePath , range , download = false } : { 
        bucket : string; 
        filePath : string; 
        range ?: string; 
        download : boolean 
        ...
        const contentType = this._getContentType(String(filePath?.split('.')?.pop()))
    ...
    private _getContentType = (extension : string) => {
    ...
    public async trashedWithFolder ({ path , bucket } : { 
        path     : string
        bucket   : string
      }) {
    ...
    public syncMetadata = async (syncBuckets: string = '*') => {
      ...
      const buckets = (this.buckets == null 
      ...
      const analyzeDirectory = async (dirPath: string) => {
          ...
          const traverseDirectory = async (currentPath: string) => {
            ...
            const items = await fsSystem.promises.readdir(currentPath).catch(_ => [])

            for(const item of items) {
  
              const itemPath = pathSystem.join(currentPath, item);
              const stats = await fsSystem.promises.stat(itemPath).catch(_ => null);
      ...
      for(const bucket of buckets) {
        ...
        fsSystem.writeFileSync(`${targetDir}/${this.metadata}`, 
          JSON.stringify({ 
            bucket , 
            info : {
    ...
    public async files (dir : string , { ignore = null} : { ignore ?: string | null } = {}) {
        const directories = await fsSystem.promises.readdir(dir, { withFileTypes: true }).catch(_ => []);
    ...
    public fileStructure = async (dirPath: string , { includeFiles = false , bucket } : { includeFiles ?: boolean , bucket : string}): Promise<any[]> => {
        const items: any[] = [];
